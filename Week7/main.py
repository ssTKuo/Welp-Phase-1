...
#定義對話登入狀態(改為JSONResponse型式)
    if 'SIGNED-IN' not in request.session or not request.session['SIGNED-IN']:
        #如果使用者未登入，則顯示JSONResponse，並以{"data": None}回應
        return JSONResponse(content={"data": None}, status_code=403)
    #如果使用者沒有輸入資料就送出請求，則顯示JSONResponse，並以{"data": None}回應
    if not username:
